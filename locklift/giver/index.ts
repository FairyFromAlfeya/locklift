...
export class Giver {
  ...
  async deployContract(
    { contract, constructorParams, initParams, keyPair },
    ...
    const { address } = await Locklift.ton.createDeployMessage({
      contract,
      constructorParams,
    ...
    await Locklift.ton.waitForRunTransaction({
      message,
      abi: contract.abi,
    });
  ...
  async setup() {
    ...
    if (this.locklift.networkConfig.giver.key) {
      const keyPair =
        await Locklift.ton.client.crypto.nacl_sign_keypair_from_secret_key({
