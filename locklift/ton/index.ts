...
export class Ton {
  ...
  async createDeployMessage({
    ...
    return Locklift.ton.client.abi.encode_message(
  ...
  async waitForRunTransaction({ message, abi }) {
    const { shard_block_id } =
      await Locklift.ton.client.processing.send_message({
        message: message.message,
        send_events: false,
      });

    return Locklift.ton.client.processing.wait_for_transaction({
      message: message.message,
      shard_block_id,
      send_events: false,
      abi: {
